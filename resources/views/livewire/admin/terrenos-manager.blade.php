<div class="space-y-6 p-4 md:p-1 transition-colors duration-500">
    ...
    <div class="animate-in fade-in duration-500">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div class="bg-white dark:bg-agri-d_bg rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 dark:border-white/5 group relative" x-data="{ menuOpen: false }">
                <div class="h-40 bg-slate-100 dark:bg-slate-800 relative overflow-hidden">
                    @if($terreno->foto_path)
                        <img src="{{ Storage::url($terreno->foto_path) }}" class="w-full h-full object-cover">
                    @else
                        <div class="w-full h-full flex items-center justify-center text-slate-300 bg-slate-50 dark:bg-white/5"><i class="fa-solid fa-mountain-sun text-6xl"></i></div>
                    @endif
                    ...
                    <button @click="window.dispatchEvent(new CustomEvent('map-center-to', { detail: { lat: {{ $terreno->latitud }}, lng: {{ $terreno->longitud }} } }))"
                            class="absolute top-4 left-4 w-9 h-9 bg-white/20 backdrop-blur-md border border-white/30 rounded-xl text-white hover:bg-agri-green transition-all shadow-lg flex items-center justify-center"><i class="fa-solid fa-crosshairs text-sm"></i></button>
                    ...
                    <div class="absolute bottom-4 left-6 right-6 flex justify-between items-end text-white">
                        <div class="min-w-0 flex-1 pr-4">
                            <h3 class="text-[18px] font-black italic truncate tracking-tight uppercase leading-none">{{ $terreno->nombre }}</h3>
                            <p class="text-[12px] font-bold uppercase italic opacity-70 truncate mt-1"><i class="fa-solid fa-location-dot mr-1"></i> {{ $terreno->ubicacion }}</p>
                        </div>
                        <div class="px-3 py-1 bg-agri-green rounded-md text-[12px] font-black italic shadow-lg shrink-0">{{ $terreno->hectareas }} ha</div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest leading-none">Información Técnica</p>
                            <div class="flex items-center space-x-1.5 text-[14px] font-bold text-slate-700 dark:text-slate-300 italic">
                                <i class="fa-solid fa-mountain text-agri-green"></i> <span>Suelo {{ ucfirst($terreno->calidad_suelo) }}</span>
                            </div>
                            <div class="flex items-center space-x-1.5 text-[14px] font-bold text-slate-700 dark:text-slate-300 italic">
                                <i class="fa-solid fa-droplet text-blue-500"></i> <span>{{ $terreno->fuente_agua }}</span>
                            </div>
                        </div>
                        <div class="space-y-2">
                            <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest leading-none">Situación Legal</p>
                            <span class="inline-block px-4 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg text-[12px] font-black text-agri-green uppercase tracking-widest italic">
